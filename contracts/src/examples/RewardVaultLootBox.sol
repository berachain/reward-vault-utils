...
contract RewardVaultLootBox is RewardVaultEntropy, ERC721 {
    ...
    ) RewardVaultEntropy(_entropyContract) ERC721(_name, _symbol) {
        baseURI = _baseURI;
        require(_rarityProbabilities.length == 5, "PL");
        require(_rarityRewardBips.length == 5, "RL");
        uint256 totalProbability = 0;
        for (uint256 i = 0; i < 5; i++) {
            rarityProbabilities[Rarity(i)] = _rarityProbabilities[i];
            rarityRewardBips[Rarity(i)] = _rarityRewardBips[i];
    ...
    function _processEntropy(
        ...
        if (pending.opener != opener) revert IO(); // Invalid Opener
        if (pending.processed) revert AP(); // Already Processed
    ...
    function openLootBox(uint256 tokenId) external {
        ...
        if (item.rarity == Rarity.COMMON && item.rewardBips == 0 && item.rewardToken == address(0) && !item.claimed) revert NM(); // Not Minted
        if (item.claimed) revert AC(); // Already Claimed
        if (ownerOf(tokenId) != msg.sender) revert NO(); // Not Owner
    ...
    function tokenURI(uint256 id) public view virtual override returns (string memory) {
        if (ownerOf(id) == address(0)) revert NM(); // Not Minted
