...
contract RewardVaultLootBox is RewardVaultEntropy {
    ...
    function createLootBox(bytes32 userRandomNumber) external payable onlyApprovedCreator returns (uint64 sequenceNumber) {
        ...
        // trusted: Pyth Entropy contract
        sequenceNumber = this.requestEntropyWithCallback{value: requiredFee}(defaultEntropyProvider, userRandomNumber, callbackData);
    ...
    function openLootBox(uint256 tokenId) external {
        ...
        if (item.rarity == LootBox.Rarity.COMMON && item.rewardBips == 0 && 
            item.rewardToken == address(0) && !item.claimed) revert NM();
    ...
    function setRarityProbabilities(LootBox.Rarity[] calldata rarities, uint256[] calldata probabilities) external onlyOwner {
